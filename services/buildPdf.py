...
def make_on_page(job_no, logo_path):
    def on_page(canvas, doc):
        ...
        if logo_path and os.path.exists(logo_path):
            canvas.drawImage(logo_path, MARGIN, h - 20*mm,
        ...
        canvas.drawRightString(w - MARGIN, h - 18*mm, "Job No. %s" % job_no)
        ...
        canvas.drawString(MARGIN, 3.5*mm,
            "Smith & Winters Electrical  |  ABN 70 626 942 596  |  Licence VIC: 26114  QLD: 91858")
        canvas.setFillColor(YELLOW)
        canvas.drawRightString(w - MARGIN, 3.5*mm,
            "Job No. %s  |  Page %d" % (job_no, doc.page))
        canvas.restoreState()
    return on_page
...
def detail_grid(rows):
    normal = ParagraphStyle("dv", fontName="Helvetica", fontSize=8.5, leading=12)
    bold   = ParagraphStyle("dk", fontName="Helvetica-Bold", fontSize=8, leading=12,
                            textColor=DARK_GREY)
    data = [[Paragraph(k, bold), Paragraph(str(v) if v else "--", normal)] for k, v in rows]
    ts = TableStyle([
...
def scope_card(quote_ref, title, description):
    ...
    content_tbl = Table([[i] for i in content_col], colWidths=[BODY_W - 5*mm],
                        style=TableStyle([
                            ("TOPPADDING",    (0,0), (-1,-1), 0),
                            ("BOTTOMPADDING", (0,0), (-1,-1), 0),
    ...
    card = Table([[marker, content_tbl]], colWidths=[5*mm, BODY_W - 5*mm],
                 style=TableStyle([
                     ("VALIGN",        (0,0), (-1,-1), "TOP"),
                     ("TOPPADDING",    (0,0), (-1,-1), 6),
                     ("BOTTOMPADDING", (0,0), (-1,-1), 6),
...
def checklist_row(text, is_included=True):
    ...
    return [Paragraph(symbol, tick_style), Paragraph(text, text_style)]
...
def build(json_path, out_path, logo_path):
    ...
    intro  = ParagraphStyle("i", fontName="Helvetica", fontSize=8.5, leading=13, textColor=DARK_GREY)
    ...
    rows = [
        ...
        ("LICENCE",      data.get("licence") or "VIC: 26114  QLD: 91858"),
    ...
    story.append(Paragraph(
        "All works below were completed by licensed electricians at %s, %s. "
        "A Certificate of Electrical Safety will be issued upon finalisation." % (site_name, site_addr),
        intro))
    story.append(Spacer(1, 3*mm))
    default_items = ["Arrived on site", "Set up safe work area and barricades", "Reviewed P.O and job notes"]
    ds = ParagraphStyle("di", fontName="Helvetica", fontSize=8.5, leading=14)
    ...
    story.append(Paragraph(
        "By signing below, both parties confirm that all works described in this report have been "
        "completed to the agreed scope and in accordance with Australian Standards. "
    ...
    sd = [
        [Paragraph("CLIENT SIGN-OFF", hs), Paragraph("SMITH &amp; WINTERS ELECTRICAL", hs)],
        [sig_col(client_name), sig_col("___________________________")]
    ]
    ...
    tys = ParagraphStyle("ty", fontName="Helvetica-Oblique", fontSize=10, leading=16,
